...
export function PartsCostSection({ parts, estimatedToPurchase }: { parts: JobPartsCost | null; estimatedToPurchase: number | null }) {
  ...
  return (
    <div className="mt-8 space-y-4">
      ...
      <div className={`${card("p-0")} max-h-[calc(var(--app-vh)_*_0.82)] overflow-auto`}>
        {filtered.length === 0 ? (
          <p className="p-8 text-center text-sm text-sdc-muted">
              ...
              {shownRows.map((row) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-sdc-border-soft/60 odd:bg-white even:bg-sdc-gray-50 hover:bg-sdc-blue-light/40">
                    <td className="sticky left-0 z-10 whitespace-nowrap bg-inherit px-2 py-1 text-sdc-muted">{l.purchaseDate ?? "—"}</td>
                    <td className="whitespace-nowrap px-2 py-1 text-sdc-muted">{l.invoicedDate ?? "—"}</td>
                    ...
                    <td className="whitespace-nowrap px-2 py-1 text-sdc-muted">{l.poNumber ?? "—"}</td>
