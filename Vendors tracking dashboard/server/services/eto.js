...
async function getAllPoDetails() {
  const db = await getPool();
  const result = await db.request()
    .query(`
      SELECT
      ...
      JOIN tblCompany c              ON poh.PurchaseSupplierID = c.CompanyID
      JOIN tblEngItemMaster eim      ON pod.ItemID = eim.ItemID
      WHERE pod.ProjectID IS NOT NULL
        AND eim.ItemCompanyID NOT IN ('Shipping', 'FEE', 'TARIFF')
      ORDER BY c.CName, pod.DateRequired
    `);
  return result.recordset;
...
// Query to get all active projects from the DB
async function getAllProjects() {
  const db = await getPool();
  // Try full query with optional columns PStatus / PType first; fall back if they don't exist
  try {
    const result = await db.request().query(`
      SELECT DISTINCT p.ProjectID, p.PDescription as ProjectName,
        p.PStatus   as Status,
        p.PType     as ProjectType,
        pa.ActTotalMaterials as ActualMaterials,
        pa.ExtendedEstimate  as Budget,
        pa.EstTotalMaterials as MaterialsBudget
      FROM vwProjects p
      LEFT JOIN vwProjectActualsVSEstimates pa ON p.ProjectID = pa.ProjectID
...
// Costing for all projects at once (hours + labor + margins)
async function getAllProjectsCosting() {
  const db = await getPool();
  const result = await db.request().query(`
    SELECT
      C.ProjectID          AS [JobID],
...
// PO details for multiple project IDs at once
async function getPoDetailsMulti(projectIds) {
  if (!projectIds || projectIds.length === 0) return [];
  if (projectIds.length === 1) return getPoDetails(projectIds[0]);
  const db = await getPool();
