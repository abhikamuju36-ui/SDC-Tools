...
var DOW_MINI     = ['S','M','T','W','T','F','S'];
var HOUR_H       = 48; // px per hour in time grid
...
var CATMAP = Object.fromEntries(CATEGORIES.map(c => [c.id, c]));
...
var parseYMD    = (s) => { const [y,m,d]=s.split('-').map(Number); return new Date(y,m-1,d); };
...
var addDays     = (d,n) => { const r=new Date(d); r.setDate(r.getDate()+n); return r; };
var daysBetween = (a,b) => Math.round((b-a)/86400000);
var startOfWeek = (d, ws=1) => { const diff=(d.getDay()-ws+7)%7; return addDays(d,-diff); };

var fmtDateLong = (d) => `${DOW_LONG[d.getDay()]}, ${MONTHS[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()}`;
var fmtDateShort= (d) => `${MONTHS_SHORT[d.getMonth()]} ${d.getDate()}`;
...
var timeToMin = (hhmm) => { if(!hhmm)return 0; const[h,m]=hhmm.split(':').map(Number); return h*60+m; };
var rotateDow = (s,arr) => arr.slice(s).concat(arr.slice(0,s));
...
// ─── Storage ─────────────────────────────────────────────────
var USER_EVENTS_KEY = 'sdc_user_events_v2';
var PREFS_KEY       = 'sdc_prefs_v2';
var EMPLOYEES_KEY   = 'sdc_employees_v1';
